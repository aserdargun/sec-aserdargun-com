...
export function ControlsPage({ locale }: { locale: Locale }) {
  ...
  const selectedControl = catalog.controlsById.get(searchParams.get('control') ?? '')
  ...
  return (
    <section className="controls-page">
      ...
      <p className="assurance-note page-note">{assuranceNote[locale]}</p>
      ...
      <p className="result-count" role="status">{controls.length} / {catalog.controls.length} {locale === 'en' ? 'controls' : 'kontrol'}</p>
      {selectedControl && <ControlDetail control={selectedControl} locale={locale} />}

  ...
  return (
    <section className="controls-page">
      ...
      <div className="filter-toolbar control-filters">
        ...
        <label><span>{locale === 'en' ? 'Control type' : 'Kontrol türü'}</span><select value={filters.type ?? ''} onChange={(event) => update('controlType', event.target.value)}><option value="">{locale === 'en' ? 'All types' : 'Tüm türler'}</option>{['prevent', 'detect', 'contain', 'recover'].map((value) => <option key={value}>{value}</option>)}</select></label>
        <label><span>{locale === 'en' ? 'Assurance' : 'Güvence'}</span><select value={filters.assurance ?? ''} onChange={(event) => update('assurance', event.target.value)}><option value="">{locale === 'en' ? 'All levels' : 'Tüm seviyeler'}</option>{['declared', 'enforced', 'observed', 'proven'].map((value) => <option key={value}>{value}</option>)}</select></label>
